...
with tab1:
    ...
    #'MaxHR'
    x8 = st.number_input('Insert a number')

    #'ExerciseAngina'
    x9 = st.radio("Choose Y or N ",ExerciseAngina_encoder.classes_)
    x9 = ExerciseAngina_encoder.transform([x9])[0]

    #'Oldpeak'
    x10 = st.slider('FastingBS', 0, 3, 5)

    #'ST_Slope'
    x11 = st.radio("Choose",ST_Slope_encoder.classes_)
    ...
    st.header('Predicted Result: ' ,Sex_encoder.inverse_transform(pred)[0])
